  ...
  return (
    <div className=" py-10 px-4 sm:px-6 md:px-8">
      ...
      <h1 className="text-2xl sm:text-3xl md:text-4xl font-bold text-center mb-10 text-[#555555]">
        Our <span className="text-[#FAB701]">Top</span> Placements
      </h1>
        ...
        {profiles.map((profile, index) => (
          <div key={index} className="flex flex-col items-center text-center space-y-2">
            <img
              src={profile.image}
              alt={profile.name}
              className="rounded-full w-20 h-20 sm:w-24 sm:h-24 md:w-28 md:h-28 lg:w-32 lg:h-32 border-4 border-[#FAB701] object-cover object-top"
            />
